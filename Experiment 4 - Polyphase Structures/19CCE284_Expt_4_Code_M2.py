...
# Shift the samples of given signal by "phase_shift" units:-
def shift_samples(signal, phase_shift):
    for i in range(phase_shift):
        signal.append(0) # Adds required zeros at the end of list
    for l in range(len(signal)-1,0,-1):
        signal[l] = signal[l-1] # Shifts the zeros to the beginning of the list
    return signal
...
print("The order of the filter is 2.")
m = 2
...
output_II = convolve_I + convolve_II
